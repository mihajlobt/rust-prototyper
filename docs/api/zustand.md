...
:warning: This technique is not recommended for adding state in React Server Components.
...
Or, just use our redux-middleware. It wires up your main-reducer, sets the initial state, and adds a dispatch function to the state slice itself and the vanilla API.
...
const usePlainStore2 = create(devtools((set) => ..., { name, store: storeName1 }))
// Usage with a redux store, it will log full action action types
