...
> **Not published as a site section.** An earlier version of this site tried to publish the library as a `/context/` collection generated by `docs/build.sh`. That script never ran as part of the actual Cloudflare Pages build, so the section 404'd (silently falling back to the homepage). It has been removed from the navigation — browse `.opencode/context/` directly in the repo instead.
