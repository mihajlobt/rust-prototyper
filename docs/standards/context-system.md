...
This page is the short version. **The full source is in [Context → Core → Context System → Overview]({{ '/context/core/context-system/overview/' | relative_url }}).**
...
The `Context` section in the sidebar is **auto-generated** from this directory on every `build.sh` run — see [Context → Overview]({{ '/context/navigation/' | relative_url }}) for the live tree.
...
- [Context → Core → Context System → Overview]({{ '/context/core/context-system/overview/' | relative_url }}) — the full 450-line source
- [Context Navigation]({{ '/context/navigation/' | relative_url }}) — the live tree
