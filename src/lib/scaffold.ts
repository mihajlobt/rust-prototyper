import { readFile, writeFile, createDir, readDir, bunInstallSync, runShellCommandSync, deleteDir, deleteFile, isNotFoundError, createSymlink } from "@/lib/ipc";
...
export async function scaffoldComponentPreview(
  ...
  // Step 9: Create symlink component-preview/src/data → ../../data
  // so that @/data imports in generated components resolve to project-level data/
  const parentDir = componentPreviewDir.substring(0, componentPreviewDir.lastIndexOf("/"));
  await ensureDataSymlink(parentDir);

  // Step 10: Patch eslint.config.js to ignore shadcn's own false-positive errors
  // and clean up stale eslint.config.ts from old scaffold versions.
  onStep?.("Patching ESLint config…");
...
export async function scaffoldScreenPreview(
  ...
  // Step 8: Create symlink screen-preview/src/screens → ../../screens
  // so that @/screens/{id}/screen resolves to the project-level screens/ directory
  await ensureScreenSymlink(projectDir);

  // Step 9: Patch vite.config.ts to allow serving files from symlinked parent dirs
  await ensureScreenPreviewViteConfig(projectDir);
...
/**
 * Ensure the screen-preview/src/screens symlink exists and points correctly.
 * The symlink resolves @/screens/{id}/screen imports in routes.ts to the
 * project-level screens/ directory. Idempotent — recreates if missing or
 * pointing to the wrong target (e.g. the old buggy ../../../screens path).
 */
export async function ensureScreenSymlink(projectDir: string): Promise<void> {
  const linkPath = `${projectDir}/screen-preview/src/screens`;
  try {
    await createSymlink(linkPath, "../../screens");
  } catch (e) {
...
/**
 * Ensure the component-preview/src/data symlink exists and points correctly.
 * The symlink resolves @/data/store imports in generated components to the
 * project-level data/ directory. Idempotent.
 */
export async function ensureDataSymlink(projectDir: string): Promise<void> {
  const linkPath = `${projectDir}/component-preview/src/data`;
  try {
    await createSymlink(linkPath, "../../data");
  } catch (e) {
    if (isNotFoundError(e)) return;
    throw e;
  }
...
/**
 * Patch the screen-preview vite.config.ts to allow serving files from outside
 * the project root (needed because the screens symlink targets ../../screens).
 * Idempotent — safe to call repeatedly.
 */
export async function ensureScreenPreviewViteConfig(projectDir: string): Promise<void> {
  const configPath = `${projectDir}/screen-preview/${P.VITE_CONFIG_TS}`;
  try {
    const config = await readFile(configPath);
    let patched = patchViteFsAllow(config);
    patched = patchViteResolveDedupe(patched);
    if (patched !== config) {
      await writeFile(configPath, patched);
    }
  } catch (e) {
