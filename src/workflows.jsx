...
// Also includes NodePalette, WorkflowsBrowser, PropertiesPanel.
...
// ─────────────────────────────────────────────────────────────
// Node component
function WorkflowNode({ node, selected, running, done, onMouseDown, onClick }) {
  const def = NODE_LOOKUP[node.type];
  if (!def) return null;
  const Ic = Icons[def.icon] || Icons.cube;
  return (
    <div
...
function WorkflowEdge({ a, b, active, done, style }) {
...
// ─────────────────────────────────────────────────────────────
// Canvas (main)
function WorkflowCanvas({ nodes, setNodes, edges, selectedId, setSelectedId, running, runState, edgeStyle }) {
  ...
  const onStageDown = (e) => {
    if (e.button === 1 || (e.button === 0 && e.target === stageRef.current) || e.target.classList?.contains('stage-bg')) {
      setSelectedId(null);
  ...
  useEffect(() => {
    const onMove = (e) => {
      if (drag) {
        ...
        const px = (e.clientX - r.left - pan.x) / pan.k;
        const py = (e.clientY - r.top  - pan.y) / pan.k;
        setNodes(ns => ns.map(n => n.id === drag.id ? { ...n, x: px - drag.dx, y: py - drag.dy } : n));
      }
      if (panning) {
...
// ─────────────────────────────────────────────────────────────
// Node Palette (left)
function NodePalette({ onDragStart, query, setQuery }) {
  return (
...
function WorkflowsBrowser({ activeTab, setActiveTab, onLoad }) {
...
function PropertiesPanel({ node, onPatch }) {
  if (!node) {
    return (
...
function DesignSystemProps({ node, onPatch }) {
...
function ThemeMiniCard({ t, selected, onSelect }) {
  return (
...
function RequirementsProps({ node }) {
  return (
    <div>
...
window.WorkflowsBrowser = WorkflowsBrowser;
